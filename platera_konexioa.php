...
$platera = $conn->query($sql);

$platerak = [];
if ($platera && $platera->num_rows > 0) {
    while ($row = $platera->fetch_assoc()) {
        $platerak[] = $row;
    }
}

$conn->close();
echo json_encode($platerak, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
...
odooraBidali('platera.json', "sale.order");
?>